...
class TencentCloud:
    def __init__(self, option, mode):
        ...
        self.option = option
        self.secret_key_id = option['way'][1]
        self.secret_key = option['way'][2]
    ...
    # 腾讯api默认接口请求频率限制：100次/秒
    # record_type支持参数'A'|'AAAA'
    def query_record(self, domain, subdomain, record_type):
        endpoint = 'dnspod.tencentcloudapi.com'
        timestamp = str(int(time.time()))

        # api查询参数，api文档:https://cloud.tencent.com/document/product/1427/56166
        params = {
        ...
        if 'Error' in response and 'RecordList' not in response:
            ...
            if error_code == 'AuthFailure.SignatureFailure':
                ...
                logging.error(
                    'record查询阶段数据有误，请查阅腾讯文档的错误代码https://cloud.tencent.com/document/product/1427/56166，详细返回值如下：')
                logging.error(mjson)
                return
        else:
            return response['RecordList'][0]['Value'], response['RecordList'][0]['RecordId']

    # tencentCloud默认该接口请求频率限制：500次/秒
    def create_record(self, domain, subdomain, record_type, ip):
        endpoint = 'dnspod.tencentcloudapi.com'
        timestamp = str(int(time.time()))

        # api查询参数，api文档:https://cloud.tencent.com/document/product/1427/56180
        params = {
            'Action': 'CreateRecord',
            'Version': '2021-03-23',
            'Domain': domain,
        ...
        if 'RecordId' not in response:
            logging.error('record创建阶段数据有误，请参阅腾讯文档https://cloud.tencent.com/document/product/1427/56180对错误码的描述，详细返回值如下：')
            logging.error(mjson)
            return
        else:
    ...
    def update_record(self, domain, subdomain, record_type, ip, domain_id):
        ...
        # api查询参数，api文档:https://cloud.tencent.com/document/product/1427/56157
        params = {
            'Action': 'ModifyRecord',
            'Version': '2021-03-23',
            'Domain': domain,
        ...
        if 'RecordId' not in response:
            logging.error(
                'record更新阶段数据有误，请参阅腾讯文档https://cloud.tencent.com/document/product/1427/56157对错误码的描述，详细返回值如下：')
            logging.error(mjson)
            return
        else:
            logging.info(subdomain+'的'+record_type+'更新成功')
    ...
    def sign_and_send(self, url, params, method: str):
        ...
        signature = self.hmac_sha1.signaturer(self.secret_key, string_to_sign)
